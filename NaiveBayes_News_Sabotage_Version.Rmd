...
rawDA$label <- rawDA$label %>% factor
class(rawDA$label)
```
```{r}
...
## Modeling & Deplpyment 
```{r}
nbayesModel <- naiveBayes(trainDTM, trainDF$article, laplace = 1)

```

```{r}
predVec <- predict(nbayesModel, testDTM)
confusionMatrix(as.matrix(predVec, testDF$article), positive = "Real", dnn = c("Prediction", "True"))
```
